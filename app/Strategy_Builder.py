...
def main():
    ...
    # Clean symbol name
    symbol = os.path.splitext(os.path.basename(symbol_file))[0]
    ...
    if "Returns" not in df.columns:
        st.error("CSV missing 'Returns' column after processing.")
        st.stop()
    ...
    if use_regime:
        ...
        if "Regime_Uptrend" not in df.columns:
            st.error("Regime filter failed to produce 'Regime_Uptrend' column.")
            st.stop()
        mask &= df["Regime_Uptrend"]

    if use_vol:
        df = apply_volatility_filter(df, quantile=vol_quantile)
        if "Vol_Filter" not in df.columns:
            st.error("Volatility filter failed to produce 'Vol_Filter' column.")
            st.stop()
        mask &= df["Vol_Filter"]
